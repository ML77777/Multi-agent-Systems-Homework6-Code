...
#Multi-agent systems homework 6 - Q-learning for example code
...
class Square:
    ...
    def Choose_action(self, epsilon,find_max=False):
        #Find the best q values that this square has and divide its probabilities. E.g In the beginning 4 q values are the same at max 0.0, thus probability is 0.25 for each
...
def print_grid(grid):
    ...
    for row_list in grid:
        for square in row_list:
            ...
            #print(square.position, square.up, square.down, square.left,square.right,end="", flush=True)
            #print(square.position, end="", flush=True)
            #print(square.q_values)
            #print("\n")
        #print("\n")
...
def q_learning():
    ...
    while not converged:
        ...
        while not reached_terminal:
            ...
            max_lookahead_action = lookahead_square.Choose_action(epsilon,True)
            #alpha = 1/(current_square.amount_action_taken[action]) 
            current_square.q_values[action] +=  alpha* (reward + GAMMA * lookahead_square.q_values[max_lookahead_action] - current_square.q_values[action])
